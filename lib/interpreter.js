...
class Interpreter {
  ...
  constructor (opts) {
    ...
    this.web3Interface._web3.eth.getBlockNumber((err, blockNumber) => {
      if (err) throw new Error(err);
      this.log(`Connected to RPC client, block height is ${blockNumber}\n`);
    });
  }
  ...
  deploy (className, args, txOptions, ast) {
    if (className in this.classes) {
      ...
      console.log(this.confirmationBlocks);
