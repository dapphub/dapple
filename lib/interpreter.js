...
class Expr {
  ...
  constructor( _value, _args, _type ){
...
class Interpreter {
  ...
  run( ast, cb ){
    ...
    var _s = JSON.stringify(ast);
    
    var interpret = function( ast ) {
      if( !(ast instanceof Expr) )  // VALUE
        return ast;
        
      if( ast.type === TYPE.SEQ ) { // SEQUENCE
        return ast.value.forEach( interpret );
      } else if( ast.type.atom ) {  // AST - ATOM
        return ast.value;
      } else {                      // AST - FUNCTION
        return ast.value.apply( self, ast.args.map( interpret ) );
      }
    }
    
    var ret = interpret( ast, internal_scope, global_scope );
    var _s2 = JSON.stringify(ast);
