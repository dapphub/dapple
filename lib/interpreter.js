...
var TYPE = {
  ...
  SYMBOL: { atom: true }, // symbol - internal reference
...
class Interpreter {
  ...
  run (ast, cb) {
    ...
    var interpret = function (ast) {
      if (!(ast instanceof Expr)) {// VALUE
        ...
        return ast.value.forEach(interpret);
    ...
    // TODO - refactor this out of the interpreter
  ...
  deploy (className, args, txOptions, ast) {
    if (className in this.classes) {
      ...
      this.log(`GAS COST: ${ parseInt(receipt.gasUsed, 16) } for "new ${ className }"\n`);
  ...
  call (varName, fName, _args, txOptions, ast) {
    // console.log(this.local[varName] instanceof Expr);
    // console.log(varName, fName, _args, txOptions, ast );
    ...
    if (this.local[varName] instanceof Expr && this.local[varName].type instanceof TYPE.OBJECT) {
      ...
      if (constant) {
        ...
        this.log(`GAS COST: ${parseInt(result.gasUsed, 16)} for "call ${ className }.${ fName }(${args.join(',')})"\n`);
      }
      ast.meta = {
        gas: result.gasUsed
      };
