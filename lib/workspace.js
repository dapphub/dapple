...
module.exports = class Workspace {
  ...
  static initialize (root_dir) {
    var dappfile = req._.extend(defaults(schemas.dappfile.schema), { name: root_dir.split(path.sep).pop() });
    fs.writeYamlSync(path.join(root_dir, constants.DAPPFILE_FILENAME), dappfile);
  ...
  addPath (path) {
    ...
    var files = filterFiles(getFiles(path));
    ...
    if( diff.length > 0 ) {
      this.dappfile.files = files.concat(this.dappfile.files);
      console.log(`added: \n  ${ diff.join('\n  ') }`);
      this.writeDappfile();
    }
  }
