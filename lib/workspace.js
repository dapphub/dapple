...
module.exports = class Workspace {
  ...
  buildDappfileTree (paths, env) {
    ...
    paths = Object.values(unq);

    // crete a map from paths to dappfiles
    var dappfilemap = {};
    paths.forEach(p => {
      dappfilemap[p] = Workspace.loadDappfile(path.join(this.package_root, path.dirname(p)));
    });

    var root = dappfilemap["dappfile"];
