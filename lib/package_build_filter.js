...
module.exports = class PackageBuildFilter {
  ...
  seed (workspace, sources) {
    ...
    let localSources = _.pickBy(sources, (_, path) => {
      return (path.startsWith(workspace.getSourcePath()) || path.startsWith('dapple')) && !path.startsWith(workspace.getPackagesPath());
    });
