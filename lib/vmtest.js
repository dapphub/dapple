...
module.exports = class VMTest {
    ...
    runInstanceTestByName(contractInstance, testFunction, cb) {
      ...
      var captureTestResults = function (err, txHash) {
        ...
        var report = function( error, message, logs, failed ) {
        ...
        var expectingAnError = function( fName ) {
          return new RegExp( DEFAULT_FAIL_FUNCTION_NAMES
                            .map( s => `^${s}$`)
                            .join('|') )
                          .test( fName );
        }

        // if expecting an error and got an error
        if( expectingAnError( testFunction ) && ( err instanceof Error ) ) {
           
          // report success
          report( null, "Passed!", [], false );
          ...
          that.web3.eth.getTransactionReceipt(txHash, function(err, receipt) {
            contractInstance.failed(function (err, failed) {
              var logs;
              if( receipt == null ) {
                failed = !expectingAnError( testFunction );
                logs = [];
