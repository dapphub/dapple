            ...
            if( receipt == null ) {
              ...
              if( Array.isArray( eventListeners ) && eventListeners.length > 0 ) {
                ...
                eventListeners.forEach( expectedEvent => {
                  // console.log(expectedEvent.args.args, logs);
                  // has to be in the logs
                  // 
                  let isPresent = logs.filter( log => log.event === expectedEvent.args.name && new RegExp( expectedEvent.args.args ).test( _.values( log.args ).join('') ) ).length > 0;

                  if( !isPresent ) message += `event ${expectedEvent.args.name} not called\n`;
