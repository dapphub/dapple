...
module.exports = class VMTest {
    ...
    // Returns a callback function for `getTransactionReceipt`
    // bound to the given contract class, test function name,
    // and callback. The returned function runs the specified
    // test on the contract at the address the receipt deployed
    // a contract to. The deployed contract is assumed to be
    // an instance of the passed-in contract class.
            ...
            that.web3.eth.getTransactionReceipt(txHash, function(err, receipt) {
                contractInstance.failed(function (err, failed) {
                    ...
                    var testResult = {
                        ...
                        logs: receipt ? that.logTranslator.translateAll(receipt.logs) : [],
                        failed: failed
                    };
                    cb(err, testResult);
                });
