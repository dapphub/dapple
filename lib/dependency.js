...
module.exports = class Dependency {
  ...
  static fromDependencyString (path, name) {
    ...
    if (name && semver.valid(path)) {
      let version = semver.clean(path);
      return new Dependency(`${name}@${version}`, version, name);
    }
  ...
  _pullDappHub (web3Settings, target) {
    ...
    if (web3Settings === 'internal') {
      ...
      try {
        web3 = req.Web3Factory.JSONRPC(web3Settings);
      } catch (e) {
    ...
    let packageHash = dapphub.objects.dapphubdb.getPackageHash.call(
        this.getName(), semver.major(this.getVersion()),
    ...
    this._pullIPFS(web3.toAscii(packageHash), target);
  }
