...
module.exports = class Dependency {
  ...
  static fromDependencyString (path, name) {
    ...
    let pathRegex = /^([^@#]+)?((#[^@#]+)?(@.+)?)$/;
    let gitPathRegex = /^(.+.git)((#[^@#]+)?(@.+)?)$/i;
    if (gitPathRegex.test(path)) {
      pathRegex = gitPathRegex;
    }
    ...
    } else if (/^(ipfs:\/\/)?Qm[A-Za-z0-9]+$/.test(path) && !version) {
      // IPS
      version = path.replace(/^ipfs:\/\//, '');
      path = 'ipfs://' + path.replace(/^ipfs:\/\//, '');
    } else if (/^@(ipfs:\/\/)?[a-z0-9]+$/i.test(version)) {
      // IPS as a version
      name = path;
  ...
  hasGitPath () {
    return (new RegExp('\\.git$', 'i')).test(this.path);
  }
