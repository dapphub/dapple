...
var BuildPipeline = function (opts) {
    ...
    var solidityFilter = filter(['*.sol', '**/*.sol'], {restore: true});
    var buildFilter = new streams.package_build_filter();
...
var BuiltClassesPipeline = function (buildDir, includeSubpackages) {
    var buildFilter = new streams.package_build_filter();
    SourcePipeline(opts).pipe(buildFilter.seed());

    return Combine(
        req.vinyl.src([buildDir + '/classes.json']),
        includeSubpackages ? req.gutil.noop() : buildFilter.filter(),
        req.buffer());
};
